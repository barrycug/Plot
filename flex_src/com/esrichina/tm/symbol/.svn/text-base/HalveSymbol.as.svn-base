package com.esrichina.tm.symbol
{
	...
	import com.esrichina.tm.geometry.Halve;

	import flash.display.Graphics;
	import flash.display.Sprite;
	import flash.geom.Point;

	public class HalveSymbol extends CustomLineSymbol
	{
		public function HalveSymbol(type:String)
		{
			super();
			this.type=type;
			this._radiu=radiu;
		...
		{
			if (geometry is Halve)
			{
				drawHalve(sprite, map, geometry as Halve);
			}
		}

		private function drawHalve(sprite:Sprite, map:Map, halve:Halve):void
		{
			if (halve.paths == null || halve.paths.length == 0)
			{
				return;
			}
